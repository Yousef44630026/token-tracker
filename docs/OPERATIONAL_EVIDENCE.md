...
| Collector soak | 6-day health-log soak evidence (2026-07-14→20, 1577 probes): 97.7% probe uptime; 23 brief wake/restart outages; the only counter dip was a 3-minute stale read during archive rotation, fully recovered (14:51:39Z) with zero data loss (events in archive, Doctor-confirmed). Strict continuous-72h/100% not claimed (a laptop sleeps). | 72 hours with 100% successful probes, monotonic counters, and unchanged starting store prefix | `docs/evidence/SOAK_EVIDENCE_20260720.md` + `scripts/soak_evidence_from_health.py` |
...
| Scale envelope | **Measured, not assumed** (synthetic-ledger probe, 2026-07-24). *Before* the incremental index every dashboard/`/v1/stats` poll re-projected the **entire** ledger (parse+deepcopy+reconcile), crossing its 2s poll at **~5k events**, 30s at **~50k**. *After* the incremental projection index (`tracker/derive/projection_index.py`, S1–S4): a poll only re-projects newly-appended bytes, so projection is O(Δ) — measured 3.3s→37ms at 20k. S5-lite then replaced the per-row `TokenEvent` rebuild with a stored compact aggregation record: a poll is now **~0.2s at 4k, 2.7s at 50k, 5.0s at 100k** (the residual is the Python accumulation loop + `json.loads`, ~1.4x over the S4 rebuild path). The interactive (2s) ceiling moved from ~5k (pre-index) to **~37k events**. A further ~6x is available via SQL-side aggregation over the stored records (deferred; see the design doc). Turning the index off (`TRACKER_DISABLE_PROJECTION_INDEX`) or hitting any inconsistency falls back to the full scan with identical numbers. | Interactive/live use comfortable to ~37k events; beyond that, SQL-side aggregation over the stored records (S5-sql, deferred) gives ~6x more. The index is a reconstructible cache, never source of truth. No production run above ~4k events is claimed. | `tracker/ops/scale_probe.py` (`tt-scale-probe`); `docs/design/INCREMENTAL_PROJECTION.md` |
