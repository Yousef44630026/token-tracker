...
| Scale envelope | **Measured, not assumed** (synthetic-ledger probe, 2026-07-24, `tracker/ops/scale_probe.py` (`tt-scale-probe`)): every dashboard/`/v1/stats` poll re-reads and re-projects the **entire** ledger — O(n) per request, no incremental/materialized rollup. Dashboard `aggregate()` crosses its own 2s poll interval at **~4–5k events**, 30s at **~45–50k**; above 50k the projection spills the whole ledger through a temp SQLite round-trip on every call (100k ≈ 80s/poll, 500k ≈ minutes). The `_IN_MEMORY_EVENT_LIMIT=50_000` constant is the wall. The signature cache keys on `(size, mtime)`, so live ingestion invalidates it every append — the live path never hits cache. The live ledger (~3.7k events) already sits at this edge. | Interactive/live use bounded to ≤ ~50k events on the current single-file design; beyond that requires a materialized rollup or `PartitionedFileRepository` with stored totals (the current SQLite index is id-lookup only, stores no totals). No production run above ~4k events is claimed. | `tracker/ops/scale_probe.py` (`tt-scale-probe`) measured table |
