...
| Scale envelope | **Measured, not assumed** (synthetic-ledger probe, 2026-07-24). *Before* the incremental index every dashboard/`/v1/stats` poll re-projected the **entire** ledger (parse+deepcopy+reconcile), crossing its 2s poll at **~5k events**, 30s at **~50k**. *After* the incremental projection index (`tracker/derive/projection_index.py`, S1–S4): a poll only re-projects newly-appended bytes, so projection is O(Δ) — measured 3.3s→37ms at 20k. The residual cost is now `aggregate()` rebuilding one `TokenEvent` per row from the index (~75µs/event, O(n)): a poll is **~0.3s at 4k, 3.7s at 50k, 7.5s at 100k**. The interactive (2s) ceiling moved from ~5k to **~27k events**. Turning the index off (`TRACKER_DISABLE_PROJECTION_INDEX`) or hitting any inconsistency falls back to the full scan with identical numbers. | Interactive/live use comfortable to ~27k events; beyond that, S5 (materialized per-bucket rollup, still to build) is needed for O(buckets) queries. The index is a reconstructible cache, never source of truth. No production run above ~4k events is claimed. | `tracker/ops/scale_probe.py` (`tt-scale-probe`); `docs/design/INCREMENTAL_PROJECTION.md` |
